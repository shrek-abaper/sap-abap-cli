...
1. **规格撰写** — 通过与 **Claude Desktop** 的对话梳理需求和实现细节，输出结构化的 Markdown 提示词文件，完整编码变更范围、验收标准和反模式约束。
2. **自主执行** — 将提示词文件直接交给智能体编码栈——**[opencode](https://opencode.ai) + [Oh My OpenAgent](https://github.com/oh-my-opencode/oh-my-openagent) + GitHub Copilot Pro**——由其自主完成规划、实现、测试和验证。
...
> 作者是拥有十余年项目从业经验的 SAP 业务顾问。这个项目源于日常工作中的实际需求，也成为了一次 AI 原生开发方式的亲身探索——希望对同样想要构建专属工具的领域专家有所参考。
