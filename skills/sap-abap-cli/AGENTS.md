...
├── scripts/
│   ├── sap_abap_cli.py   # CLI entry point — Click @cli.command definitions only
...
| Task | File |
|------|------|
| Add new SAP object command | `handlers.py` (logic) + `sap_abap_cli.py` (Click wrapper) |
...
## CONVENTIONS
- `sap_abap_cli.py` is a thin Click shell — no business logic, just `_output(handlers.xxx())` calls
- Every handler returns `AdtResult(text, is_error)` — never raises, never prints directly
- New commands: add handler in `lib/handlers.py` → add `@cli.command` in `sap_abap_cli.py` → document in `SKILL.md`
- Output format by command type: source code → plain text, DDIC/search/transaction → raw XML, `get-package` → JSON

## ANTI-PATTERNS
- Do NOT add business logic to `sap_abap_cli.py` Click commands — logic goes in `handlers.py`
